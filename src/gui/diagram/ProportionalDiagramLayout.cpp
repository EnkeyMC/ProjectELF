...
void ProportionalDiagramLayout::layoutNodes() {
    ...
    this->forEachNode([&maxStretchRatio](DiagramNode &node) {
        if (node.getProportionalSize() * MIN_HEIGHT < node.getMinHeight()
                && node.getProportionalSize() > 0.0001)
        {
            if (node.getMinHeight() / static_cast<double>(node.getProportionalSize() * MIN_HEIGHT) > maxStretchRatio) {
                maxStretchRatio = static_cast<double>(node.getMinHeight()) / node.getProportionalSize() * MIN_HEIGHT;
            }
        }
    });
